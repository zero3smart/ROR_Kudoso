module Api
  module V1
    class DevicesController < ApiController
      ...
      param :udid, String, desc: "Unique identfier for device (generated by MDM)"
      ...
      def deviceDidRegister
        ...
        if device.nil?
          ...
          if auth != Digest::MD5.hexdigest(request.query_string + device.device_token.reverse)
      ...
      api :PATCH, "v1/devices/:uuid/status", "Updates state information from the MDM regarding device"
      param :udid, String, desc: "Unique identfier for device (generated by MDM)"
      ...
      def status
        ...
        if device.nil?
          ...
          if device.expires_at.present? and device.expires_at < Date.today
            ...
            begin
              @device = Device.find_by_uuid(params[:uuid])
              if @device.udid == params[:udid]
                if params[:lastReachedAt]
                  messages[:error] << @device.errors.full_messages unless @device.update_attribute(:last_contact, Time.at(params[:lastReachedAt]))
                end
                ...
                messages[:error] << 'The device UDID did not match!'
              end
