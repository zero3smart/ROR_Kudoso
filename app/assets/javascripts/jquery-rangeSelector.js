            ...
            $(this).mousedown(function (e) {
                ...
                for (var x in timeBlocks) {
                    ...
                    if (Array.isArray(times) && times.length == 2) {
                        if (times[1] < startBlock) {
                            console.log('Found earlier block: ' + times[0] + ':' + times[1]);
                            idx++;
                        } else if (times[0] <= startBlock && times[1] >= startBlock) {
                            ...
                            break;
                        } else if (times[0] < endBlock) {
                            endBlock = times[0];
                            end = endBlock * base;
                            console.log('Would overrun next block, attempting to adjust to ' + startBlock + ', ' + settings.controlsSize + ', ' + endBlock);
                            if (startBlock + (settings.controlsSize * 2) <= endBlock) {
                                ...
                                divArray.splice(idx, 0, div);
                                var elm = [startBlock, endBlock];
                                timeBlocks.splice(idx, 0, elm);
                            } else {
                                console.log('can not make this block due to proximity');
                                div = 1;
                            }
                ...
                if (div == null) {
                    ...
                    $that.append(div);
                    divArray.splice(idx, 0, div);
                    var newElm = [startBlock, endBlock];
